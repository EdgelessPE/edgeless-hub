  ...
  <div>
    <a-layout id="main" :style="{ height: '100%' , width:'100%'}">
      ...
      <a-layout>
        ...
        <a-layout-content
            :style="{ margin: '24px 16px', padding: '24px', background: '#fff', overflow: 'auto', height: '80vh'}"
        >
          <router-view/>
        </a-layout-content>
      </a-layout>
  ...
  methods: {
    ...
    refreshData(successNotification) { //将分类数据推至store，然后调用getPluginData()
      try {
        ...
        this.$message.error({
          content: '刷新镜像站索引失败，镜像站接口' + this.$store.state.stationObject.name + "错误：" + err.message,
          key: 'updateMirror'
        })
      }
