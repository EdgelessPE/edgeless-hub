...
<a-row style="width: 100%">
    ...
    <a-result :title="event_selected.DisplayTitle">
      <template slot="subTitle">
        <template v-if="showTags" v-for="item in event_selected.Tags">
          <a-tag :color="item.color" v-on:click="$router.push(encodeURI('/wiki?location=https://wiki.edgeless.top/v2/global/log.html#edgeless核心更新日志-当前已发布最新alpha版本-edgeless-alpha'))">
            {{item.text}}
          </a-tag>
        </template>
        <br/>
  ...
  <a-col :span="8">
    <a-card :title="$store.state.fileList.length+$store.state.updateList.length===0?'未安装插件':($store.state.updateList.length===0?('已安装'+$store.state.fileList.length+'个插件'):('有'+$store.state.updateList.length+'个插件可更新'))" :bordered="false" style="width: 300px">
      <template slot="extra">
