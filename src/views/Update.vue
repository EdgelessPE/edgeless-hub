<template>
  <div>
    ...
    <div class="steps-content" key="0" v-if="$store.state.UpdateInfo.state===0||$store.state.UpdateInfo.state===1">
      <a-result :title="$store.state.UpdateInfo.state===0?'有可用的Edgeless更新':'正在下载升级文件'"
                :subTitle="'当前版本：'+localVersion+' >> 最新版本：'+onlineVersion">
        <template #icon>
        ...
        <template #extra>
          <a-space direction="vertical" style="width: 100%">
            ...
            <a-button v-else-if="$store.state.UpdateInfo.state===0" type="primary" v-on:click="startDownload"
                      :loading="loading">
              开始
            </a-button>
          </a-space>
