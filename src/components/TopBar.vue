<template>
  <a-row>
    <a-col span="1"/>
    <a-col span="1">
      <a-avatar v-if="showBackButton" class="button" icon="arrow-left" style="color: #108ee9; backgroundColor: #ffffff"
    ...
    <a-col span="1">
      <a-popover :title="(downloadingTasks===0)?'无下载任务':globalSpeed" placement="bottomRight">
        <template slot="content">
          <TaskNodeNaive v-for="(item) in $store.state.tasks[0]" :name="compressString(item.name)"
